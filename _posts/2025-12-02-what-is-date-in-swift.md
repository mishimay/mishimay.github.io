...
- UTC（協定世界時）
  - TAIと同じ正確な秒数を刻むが、UT1とのずれが0.9秒以上大きくなると閏秒が追加または削除される
    - 今後しきい値を0.9秒より大きくして閏を追加または削除する頻度を下げる話が出ている
      - 数秒になる？　閏分？　閏時間？
...
- 月の朔弦望時間の計算や二十四節気の日時の計算に必要
  - 月と太陽と地球との位置関係はTTによって計算されるが、それが地球上で何時何分何秒になるかはUT1に依るため（と理解しています）
- 実際に問題になるケースとしては、旧暦（日本における太陰太陽暦、特に天保暦）の未来の日付の決定ができない
  - 朔を含む日を各月の一日とするため
  - 閏月の決定に二十四節気を用いるため
...
- 閏秒のしきい値が大きくなって閏分や閏時間が導入された場合、現在のDateでは対応しきれなくなるケースが増えそう
  - 仮に1日25時間の日ができたときに1時間丸ごと無視するのは無理がある
  - Leap Smear を使うとUTCとの差が大きくなりすぎる
- Dateに何かプロパティが追加されたり新しい構造体やクラスが導入される可能性
...
- うるう秒がなくなる？ <https://eco.mtk.nao.ac.jp/koyomi/wiki/BBFEB9EF2FA4A6A4EBA4A6C9C3A4ACA4CAA4AFA4CAA4EBA1A9.html>
